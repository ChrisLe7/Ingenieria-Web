...
<jsp:useBean  id="infoTransaccionesBizum" scope="session" class="es.uco.iw.display.InfoPagosBizumBean"></jsp:useBean>
...
<%@ page import ="java.util.ArrayList , es.uco.iw.negocio.bizum.BizumDTO" %>
...
}else if(infoTransaccionesBizum.getPagos().isEmpty()){
	System.out.println("Error");
%> 

<p>
AUN NO HA REALIZADO NINGÚN BIZUM
</p>

<% }else{ 
	System.out.println("Error2");
ArrayList<BizumDTO> ListaTransacciones = new ArrayList<BizumDTO>();
ListaTransacciones = infoTransaccionesBizum.getPagos();

for(BizumDTO transaccion : ListaTransacciones){
%>
...
<div class="transaccion">
	...
	Telefono Origen: <%=transaccion.getTelefonoOrigen()%><br/>
	
	Telefono Destino: <%=transaccion.getTelefonoDestino()%><br/>
	...
	<form>
		...
		<input type="hidden" name="idCuentaOrigen" value="<%=transaccion.getTelefonoOrigen()%>">
		<input type="hidden" name="idCuentaDestino" value="<%=transaccion.getTelefonoDestino()%>">
		<input type="hidden" name="idCuentaSeleccionada" value="<%=infoTransaccionesBizum.getIdCuenta()%>">
		<input type="hidden" name="idTelefonoSeleccionado" value="<%=infoTransaccionesBizum.getTelefono()%>">
	...
	<div class="overlay" id="overlay">
		...
		<div class="popup" id="popup">
			...
			<form class="transaccion" method="post" action="" id="filter">
				...
				<input type="hidden" name="idCuenta" value="<%=infoTransaccionesBizum.getIdCuenta()%>">
